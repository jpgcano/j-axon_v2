...
export class AssetActionsController {
  constructor(private anullationAssetService: AnullationAssetService) {}

  async voidAsset(req: Request, res: Response) {
    const { id } = req.params;
    const { reason } = req.body;
    const actorId = (req as any).user?.id || 'ADMIN-USER-001';
