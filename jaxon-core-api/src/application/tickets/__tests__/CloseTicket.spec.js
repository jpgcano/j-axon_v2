...
describe('CloseTicket', () => {
    ...
    it('should throw if ticket not found', async () => {
        ...
        await expect(useCase.execute({ id: 'missing', actorId: 'user-1', userRole: 'ADMIN' })).rejects.toBeInstanceOf(NotFoundException);
    });
    it('should throw if role cannot modify', async () => {
        const ticket = Ticket.create('ticket-1', 'asset-1', 'Desc', 1, 1, 'user-1');
        ticketRepository.findById.mockResolvedValue(ticket);
        await expect(useCase.execute({ id: 'ticket-1', actorId: 'user-1', userRole: 'AUDITOR' })).rejects.toBeInstanceOf(InvalidArgumentException);
    });
});
//# sourceMappingURL=CloseTicket.spec.js.map