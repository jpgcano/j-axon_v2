...
describe('AssignTicket', () => {
    ...
    it('should assign ticket to tech and audit', async () => {
        const ticket = Ticket.create('ticket-1', 'asset-1', 'Desc', 1, 1, 'user-1');
    ...
    it('should throw if ticket not found', async () => {
        ...
        await expect(useCase.execute({ id: 'missing', techId: 'tech', actorId: 'user-1', userRole: 'ADMIN' })).rejects.toBeInstanceOf(NotFoundException);
    });
    it('should throw if role cannot modify', async () => {
        const ticket = Ticket.create('ticket-1', 'asset-1', 'Desc', 1, 1, 'user-1');
        ticketRepository.findById.mockResolvedValue(ticket);
        await expect(useCase.execute({ id: 'ticket-1', techId: 'tech', actorId: 'user-1', userRole: 'AUDITOR' })).rejects.toBeInstanceOf(InvalidArgumentException);
    });
});
//# sourceMappingURL=AssignTicket.spec.js.map